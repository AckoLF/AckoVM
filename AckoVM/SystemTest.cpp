...
SystemTest::SystemTest(System &system_, void *processVMSpace,
                       PageNum processVMSpaceSize)
    : mutex(),
      system(system_),
      beginSpace(processVMSpace),
      endSpace(
          (void *)((uint8_t *)beginSpace + PAGE_SIZE * processVMSpaceSize)) {}

Status SystemTest::doInstruction(
    Process &process,
  ...
  for (auto iter = addresses.begin(); iter != addresses.end(); iter++) {
    ...
    switch (accessType) {
      ...
      case EXECUTE: {
        ...
        char value;
        Status success =
            system.access(process.getProcessId(), address, accessType);
        if (success != OK) {
...
std::mutex &SystemTest::getGlobalMutex() { return mutex; }
