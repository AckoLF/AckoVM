...
RandomNumberGenerator<Number>::RandomNumberGenerator(int seed)
    : randomGenerator(seed), mutex() {}

template <typename Number>
Number RandomNumberGenerator<Number>::getRandomNumber(
    const typename RandomNumberGenerator::NumberLimits &limits) {
...
template <typename Number>
std::vector<Number> RandomNumberGenerator<Number>::getRandomNumbers(
    const typename RandomNumberGenerator::NumberLimits &limits, int number) {
  std::lock_guard<std::mutex> guard(mutex);
...
template <typename Number>
Number RandomNumberGenerator<Number>::getRandomNumberNonThreadSafe(
    const typename RandomNumberGenerator::NumberLimits &limits) {
