...
ProcessTest::ProcessTest(System &system, SystemTest &systemTest_)
  ...
  if (OK != addCodeSegment(address, size)) {
    std::cout << "Cannot create code segment in process "
              << process->getProcessId() << std::endl;
    throw std::exception();
  }
  ...
  for (int i = 0; i < 10; i++) {
    ...
    if (OK != addDataSegment(address, size)) {
      std::cout << "Cannot create data segment in process "
                << process->getProcessId() << std::endl;
      throw std::exception();
    }
  }
...
Status ProcessTest::addCodeSegment(VirtualAddress address, PageNum size) {
  ...
  checkMemory.emplace_back(
      std::make_tuple<MemoryBackup, VirtualAddress, PageNum>(
...
void ProcessTest::run() {
  ...
  for (int i = 0; i < (1 << POWER_OF_NUMBER_OF_INSTRUCTIONS); i++) {
    for (int j = 2; j < checkMemory.size(); j++) {
      ...
      if (status != OK) {
        std::cout << "Instruction in process " << process->getProcessId()
                  << " failed.\n";
...
bool ProcessTest::isFinished() const { return finished; }
